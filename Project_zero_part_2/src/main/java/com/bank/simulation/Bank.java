		...
		while (true) {
			...
			case "withdraw":
				...
				if (bank.getAuthority() == 1) {
					if (qty1 > account2.getBalance()) {
						System.out.println(
								"Balance " + account2.getBalance().toString() + ", can't withdraw " + qty1.toString());
						break;
					}
					...
					if (qty1 > bank.GetAccount(account2.getJoinName()).getBalance()) {
						System.out.println("Balance " + bank.GetAccount(account2.getJoinName()).getBalance().toString()
								+ ", can't withdraw " + qty1.toString());
						break;
					}
			...
			case "transfer":
				...
				if (qty2 > account3.getBalance()) {
					System.out.println(
							"Balance " + account3.getBalance().toString() + ", can't transfer " + qty2.toString());
					break;
				}
				...
				for (Account account4 : bank.getAccounts()) {
					if (account4.getAuthority() == 1) {
						...
						if (account4.getState() == 0)
							...
							stat = "ALLOW";
						System.out.println("authority:" + AUTH[account4.getAuthority()] + ",name:" + account4.getName()
								+ ", password:" + account4.getPassword() + ", balance:"
								+ account4.getBalance().toString() + ", state:" + stat);
					}
				}
				break;
			case "deny":
			...
			case "cancel":
				...
				break;
			// admin and employee accounts can only use this option to deposit customer
			// accounts
			case "depositx":
				if (bank.getAuthority() != 4) {
					System.out.println("Invalid authority!");
				...
				System.out.println(cmd[1] + " account " + qty4.toString() + " deposited. Current balance is "
						+ new_qty4.toString());
				break;
			// admin and employee accounts can only use this option to withdraw from
			// customer accounts
			case "withdrawx":
				if (bank.getAuthority() != 4) {
					System.out.println("Invalid authority!");
				...
				if (qty5 > account5.getBalance()) {
					System.out.println(
							"Balance " + account5.getBalance().toString() + ", can't withdraw " + qty5.toString());
					break;
				}
				Double new_qty5 = bank.Withdraw(cmd[1], qty5);
				System.out.println(cmd[1] + " account " + qty5.toString() + " withdrawed. Current balance is "
						+ new_qty5.toString());
				break;
