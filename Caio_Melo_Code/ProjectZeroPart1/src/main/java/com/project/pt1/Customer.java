...
public class Customer{
	...
	public Customer(int a, String b, String c,double f, String stat,String jstat, int jID) {
		...
		this.password = c;
		this.funds = 0;
		if(stat.equalsIgnoreCase("PENDING")) { //Convert from string value in SQL to the enumeration declared here.
			...
			try {
				...
				for(Customer temp:cust) {
					if(temp.getUsername().contentEquals(x) && temp.getPassword().contentEquals(y)) {
						...
						MainDriver.cDAO.updateCustomer(temp); //SQL
	...
	public String toString() {
		return "Name: [" + this.username +"] " +
				...
				+ "Joint Status: [" + this.getJStatus().toString() + "]";
	}
	public String toDetailedString() {
		return "Name: [" + this.username +"] " 
				+ " AccountID: [" + this.bankID+ "] Funds: $["+  this.getFunds()+  "] Application Status: [" + this.appStatus+"] Joint Status: ["+this.jointStatus + "]";
	}
	...
	public void moveMoney(Customer x) {
		...
		if(this.getFunds() <trans) {
			...
			MainDriver.cDAO.updateCustomer(x); //SQL STUFF!!
			MainDriver.cDAO.updateCustomer(this);
