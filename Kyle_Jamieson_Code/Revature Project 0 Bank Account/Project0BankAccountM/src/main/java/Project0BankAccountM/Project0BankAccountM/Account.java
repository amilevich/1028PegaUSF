...
public class Account implements Serializable
{
	...
	// In cents to avoid rounding errors or weirdness from partial cents in the account
...
//		m_password2 = new String("");
	...
	{
		super();
		this.m_nAccountNumber = _nAccountNumber;
		this.m_nFlags = _nFlags;
		...
		this.m_nSSN1 = _nSSN1;
		this.m_nSSN2 = _nSSN2;
		this.setSave(_nSavingBalance);
		this.setCheck(_nCheckBalance);
		this.setLoan(_nLoanBalance);
	}

	// Names of the flags that the call can store
	...
	// Set the m_nFlags to the input value.
	// WARNING only AccountDaoImpt should be using this. all other classes should use setFlag instead
	public void setFlagsDao(short _input)
	{
		m_nFlags = _input;
	}
	
	public void setSave(double _input)
	{
		m_nSavingBalance = (long)(_input * 100); 
	}

	public void setCheck(double _input)
	{
		m_nCheckBalance = (long)(_input * 100); 
	}

	public void setLoan(double _input)
	{
		m_nLoanBalance = (long)(_input * 100); 
	}

	public boolean depositSave(double _dAmount)
	{
		if(!isAccountActive())
		{
	...
	public boolean depositCheck(double _dAmount)
	{
		if(!isAccountActive())
		{
			System.out.println("ERROR: Account not active. Plase call (123) 123-4567 for details");
	...
	public boolean withdrawSave(double _dAmount)
	{
		if(!isAccountActive())
		{
			System.out.println("ERROR: Account not active. Plase call (123) 123-4567 for details");
	...
	public boolean withdrawCheck(double _dAmount)
	{
		if(!isAccountActive())
		{
			System.out.println("ERROR: Account not active. Plase call (123) 123-4567 for details");
...
//		System.out.println("Change amount: " + nWithAmount);
	...
	public boolean transferFromSave(double _dAmount, int _nTargetAccount)
	{
	...
	// Transfer money between accounts within the bank
	public boolean transferToSave(double _dAmount, int _nTargetAccount)
	{
		if(_nTargetAccount != m_nAccountNumber)
		{
