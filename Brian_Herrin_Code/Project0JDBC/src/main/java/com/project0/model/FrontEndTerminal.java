...
public class FrontEndTerminal extends Terminal {
			...
			if(DataBaseAccounts.dBAccs.isAccountId(fromAccount)) {
				if(DataBaseAccounts.dBAccs.isAccountId(toAccount)) {
					if(transBalance < DataBaseAccounts.dBAccs.getMap(fromAccount).getBalance()) {
						...
						System.out.println("Transfer Complete: $" + DataBaseAccounts.dBAccs.getMap(fromAccount).getBalance() + " $" + DataBaseAccounts.dBAccs.getMap(toAccount).getBalance());
		...
		if(isYourAccount) {
			if(DataBaseAccounts.dBAccs.isAccountId(fromAccount)) {
				if(withdrawBalance < DataBaseAccounts.dBAccs.getMap(fromAccount).getBalance()) {
					...
					System.out.println("Withdraw Complete: $" + DataBaseAccounts.dBAccs.getMap(fromAccount).getBalance());
					//DataBaseAccounts.serializeDataBase();
				} else { System.out.println("Not enough money"); }
	...
	private static void customerDepositScreen(Scanner sc, Customer c) {
		...
		if(isYourAccount) {
			if(DataBaseAccounts.dBAccs.isAccountId(toAccount)) {
				...
				System.out.println("Deposit Complete: $" +DataBaseAccounts.dBAccs.getMap(toAccount).getBalance());
				//DataBaseAccounts.serializeDataBase();
	...
	public static void accountApplicationScreen(){
		...
		BankDaoImpl.dBDao.insertCustomer(newC);
		BankDaoImpl.dBDao.insertAccount(newA);
